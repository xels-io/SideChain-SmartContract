...
namespace Xels.Bitcoin.IntegrationTests
{
    ...
    /// </summary>
    public class XlcOverrideRegTest : XlcRegTest
    {
        public XlcOverrideRegTest() : base()
        {
    ...
    public class SegWitTests
    {
        ...
        [Fact]
        public void SegwitActivatedOnXlcNode()
        {
            using (NodeBuilder builder = NodeBuilder.Create(this))
            {
                var network = new XlcOverrideRegTest();

                // Set the date ranges such that segwit will 'Start' immediately after the initial confirmation window.
                network.Consensus.BIP9Deployments[XlcBIP9Deployments.Segwit] = new BIP9DeploymentsParameters("Test", 1, 0, DateTime.Now.AddDays(50).ToUnixTimestamp(), 8);
        ...
        public void MineSegwitBlock()
        {
            using (NodeBuilder builder = NodeBuilder.Create(this))
            {
                ...
                CoreNode node = builder.CreateXelsPosNode(KnownNetworks.XlcRegTest).Start();
        ...
        public void StakeSegwitBlock()
        {
            ...
            {
                // Even though we are mining, we still want to use PoS consensus rules.
                Network network = KnownNetworks.XlcRegTest;
                ...
                var staker = node.FullNode.NodeService<IPosMinting>() as XlcMinting;
        ...
        public void CheckSegwitP2PSerialisationForWitnessNode()
        {
            ...
            {
                CoreNode node = builder.CreateXelsPosNode(KnownNetworks.XlcRegTest).Start();
                CoreNode listener = builder.CreateXelsPosNode(KnownNetworks.XlcRegTest).Start();
        ...
        public void CheckSegwitP2PSerialisationForNonWitnessNode()
        {
            using (NodeBuilder builder = NodeBuilder.Create(this))
            {
                ...
                var network = new XlcRegTest();
                network.SetPrivatePropertyValue("Name", "XlcRegTestWithDeployments");
                Assert.NotNull(network.Consensus.BIP9Deployments[2]);

                var networkNoBIP9 = new XlcRegTest();
                networkNoBIP9.SetPrivatePropertyValue("Name", "XlcRegTestWithoutDeployments");
        ...
        public void SegwitWalletTransactionBuildingAndPropagationTest()
        {
            using (NodeBuilder builder = NodeBuilder.Create(this))
            {
                CoreNode node = builder.CreateXelsPosNode(KnownNetworks.XlcRegTest).WithWallet().Start();
                CoreNode listener = builder.CreateXelsPosNode(KnownNetworks.XlcRegTest).WithWallet().Start();
