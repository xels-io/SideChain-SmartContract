...
namespace Xels.Bitcoin.IntegrationTests.Wallet
{
    public class OfflineSigningTests
    {
        ...
        public async Task SignTransactionOffline()
        {
            ...
            {
                CoreNode miningNode = builder.CreateXelsPosNode(this.network).WithReadyBlockchainData(ReadyBlockchain.XlcRegTest150Miner).Start();
        ...
        public async Task SignColdStakingSetupOffline()
        {
            using (NodeBuilder builder = NodeBuilder.Create(this))
            {
                CoreNode miningNode = builder.CreateXelsColdStakingNode(this.network).WithReadyBlockchainData(ReadyBlockchain.XlcRegTest150Miner).Start();
