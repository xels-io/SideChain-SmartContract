...
namespace Xels.Features.FederatedPeg.Distribution
{
    ...
    public sealed class RewardDistributionManager : IRewardDistributionManager
    {
        ...
        public List<Recipient> DistributeToMultisigNodes(int blockHeight, Money fee)
        {
            ...
            this.logger.Info("Fee reward to multisig node at main chain height {0} will distribute {1} XLC between {2} multisig mining keys.", blockHeight, fee, multiSigMinerScripts.Count);
            ...
            foreach (Script multiSigMinerScript in multiSigMinerScripts)
            {
                if (multiSigMinerScript.IsScriptType(ScriptType.P2PK))
                {
                    ...
                    this.logger.Debug($"Paying multisig member '{pubKey}' {feeReward} XLC.");
                }
                else
                {
                    multiSigRecipients.Add(new Recipient() { Amount = feeReward, ScriptPubKey = multiSigMinerScript });
                    this.logger.Debug($"Paying multisig member '{multiSigMinerScript.ToHex()}' (hex) {feeReward} XLC.");
                }
        ...
        private List<Recipient> ConstructRecipients(int heightOfRecordedDistributionDeposit, long totalBlocks, Money totalReward)
        {
            ...
            */
            ...
            this.logger.Info("Reward distribution at main chain height {0} will distribute {1} XLC between {2} mining keys.", heightOfRecordedDistributionDeposit, totalReward, recipients.Count);
