...
namespace Xels.Features.FederatedPeg.IntegrationTests
{
    ...
    {
        private class XlcRegTestAdjusedCoinbaseMaturity : XlcRegTest
        {
            public XlcRegTestAdjusedCoinbaseMaturity()
            {
        ...
        public void RewardsToSideChainCanBeBatched()
        {
            ...
            for (int height = 23; height <= 30; height++)
            {
                ...
                TxOut ccDummyAddressSingle = rewardTransaction.Outputs[0];
                Assert.Equal(XlcCoinstakeRule.CcTransactionTag(network.CcRewardDummyAddress), ccDummyAddressSingle.ScriptPubKey);
            ...
            // The first block to count towards batched rewards would be at height 30, so that will 10 x 9 (90) XLC.
            Assert.Equal(Money.Coins(90), block41.Transactions[2].Outputs[1].Value);
            TxOut ccDummy = block41.Transactions[2].Outputs[0];
