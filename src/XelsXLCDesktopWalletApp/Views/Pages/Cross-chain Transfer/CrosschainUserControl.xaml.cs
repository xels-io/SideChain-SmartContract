        ...
        private async Task SendTransactionAsync(TransactionSending tranSending)
        {
            try
            {
                if (isValid())
                {
                    ...
                    {
                        this.isSending = false;
                        ErrorModel errors = JsonConvert.DeserializeObject<ErrorModel>(content);

                        foreach (ErrorModel error in errors.Errors)
                        {
                            MessageBox.Show(error.Message);
                            break;
                            //MessageBox.Show("Error Code" + response.StatusCode + " : Message - " + response.ReasonPhrase);
            ...
            try
            {
                if (isValid())
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        //this.NavigationService.Navigate(new SendConfirmationMainChain(sendConfirmation, this.walletName));
                        this.TransactionBuilding.Password = "";
                        this.DestinationAddressText.Clear();
                        this.SendAmountText.Clear();
                        this.TransactionFeeText.Clear();
                        this.password.Clear();
                    }
                    else
                    {
