...
namespace Xels.Features.SQLiteWalletRepository.Tables
{
    internal class HDTransactionData
    {
        ...
        internal static IEnumerable<FlattenedHistoryItem> GetHistory(DBConnection conn, int walletId, int accountIndex, int limit, int offset, string txId, string address, bool forSmartContracts = false, bool forCc = false)
        {
            string strLimit = DBParameter.Create(limit);
            ...
            string receives = forSmartContracts ? "" : $@"
                    ...
                    AND     (t.OutputTxIsCoinbase != 0 OR t2.SpendTxId IS NULL){(!forCc ? "" : $@"
