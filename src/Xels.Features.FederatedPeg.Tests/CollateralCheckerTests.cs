...
namespace Xels.Features.FederatedPeg.Tests
{
    public class CollateralCheckerTests
    {
        ...
        private void InitializeCollateralChecker([CallerMemberName] string callingMethod = "")
        {
            ...
            FederatedPegSettings fedPegSettings = FedPegTestsHelper.CreateSettings(network, KnownNetworks.XlcRegTest, dataFolder, out NodeSettings nodeSettings);

            var counterChainSettings = new CounterChainSettings(nodeSettings, new CounterChainNetworkWrapper(Networks.Xlc.Regtest()));
