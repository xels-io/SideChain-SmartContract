...
{
    // TODO: These tests can be removed after successful XLC go-live.
    public class VerifyMultisigAddresses
    {
        ...
        public void CheckMultisigAddresses()
        {
            ...
            var networks = new Network[] { new XlcMain(), new XlcTest(), new XlcRegTest() };
        ...
        public void CheckBech32Prefixes()
        {
            ...
            // in the FederationSetup tool in the legacy Xels (SBFN) code-base!!!
            const byte xlcMainScriptAddressPrefix = 140;
            const byte xlcTestScriptAddressPrefix = 127;
            const byte xlcRegTestScriptAddressPrefix = 127;

            Assert.Single(KnownNetworks.XlcMain.Base58Prefixes[(int)Base58Type.SCRIPT_ADDRESS]);
            Assert.Equal(xlcMainScriptAddressPrefix, KnownNetworks.XlcMain.Base58Prefixes[(int)Base58Type.SCRIPT_ADDRESS][0]);
            Assert.Single(KnownNetworks.XlcTest.Base58Prefixes[(int)Base58Type.SCRIPT_ADDRESS]);
            Assert.Equal(xlcTestScriptAddressPrefix, KnownNetworks.XlcTest.Base58Prefixes[(int)Base58Type.SCRIPT_ADDRESS][0]);
            Assert.Single(KnownNetworks.XlcRegTest.Base58Prefixes[(int)Base58Type.SCRIPT_ADDRESS]);
            Assert.Equal(xlcRegTestScriptAddressPrefix, KnownNetworks.XlcRegTest.Base58Prefixes[(int)Base58Type.SCRIPT_ADDRESS][0]);
        }
    }
}
