...
namespace Xels.Bitcoin.Tests.Base
{
    ...
    {
        public ChainRepositoryTest() : base(KnownNetworks.XlcRegTest)
        {
        }
        ...
        public void SaveChainToDisk()
        {
            ...
            var chain = new ChainIndexer(KnownNetworks.XlcRegTest);
        ...
        public void LoadChainFromDisk()
        {
            ...
            using (var engine = new DB(new Options { CreateIfMissing = true }, new DataFolder(dir).ChainPath))
            {
                using (var batch = new WriteBatch())
                {
                    ...
                    foreach (ChainedHeader block in blocks)
                    {
                        ...
                        ConsensusFactory consensusFactory = KnownNetworks.XlcRegTest.Consensus.ConsensusFactory;
            ...
            {
                var testChain = new ChainIndexer(KnownNetworks.XlcRegTest);
