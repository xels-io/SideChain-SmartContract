...
namespace Xels.Bitcoin.Tests.Base
{
    public class ConsensusManagerBehaviorTestsHelper
    {
        ...
        {
            // Chain
            var chain = new ChainIndexer(KnownNetworks.XlcMain);
            ...
            connectionManagerMock.SetupGet(x => x.ConnectionSettings).Returns(new ConnectionManagerSettings(new NodeSettings(KnownNetworks.XlcMain)));
        ...
        private Mock<INetworkPeer> CreatePeerMock()
        {
            ...
            var connection = new NetworkPeerConnection(KnownNetworks.XlcMain, peer.Object, new TcpClient(), 0, (message, token) => Task.CompletedTask,
            ...
            VersionPayload version = connectionParameters.CreateVersion(new IPEndPoint(1, 1), new IPEndPoint(1, 1), KnownNetworks.XlcMain, new DateTimeProvider().GetTimeOffset());
