...
namespace Xels.Bitcoin.Tests.Consensus
{
    public class CheckPointsTest
    {
        ...
        {
            var checkpoints = new Checkpoints(KnownNetworks.TestNet, new ConsensusSettings(NodeSettings.Default(KnownNetworks.XlcTest)) { UseCheckpoints = true });
        ...
        {
            var checkpoints = new Checkpoints(KnownNetworks.RegTest, new ConsensusSettings(NodeSettings.Default(KnownNetworks.XlcTest)) { UseCheckpoints = true });
        ...
        [Fact]
        public void GetLastCheckPointHeight_XlcMainnet_ReturnsLastCheckPointHeight()
        {
            var checkpoints = new Checkpoints(KnownNetworks.XlcMain, new ConsensusSettings(NodeSettings.Default(KnownNetworks.XlcTest)) { UseCheckpoints = true });

            int result = checkpoints.GetLastCheckpointHeight();

            Assert.Equal(600_000, result);
        }

        [Fact]
        public void GetLastCheckPointHeight_XlcTestnet_ReturnsLastCheckPointHeight()
        {
            var checkpoints = new Checkpoints(KnownNetworks.XlcTest, new ConsensusSettings(NodeSettings.Default(KnownNetworks.XlcTest)) { UseCheckpoints = true });

            int result = checkpoints.GetLastCheckpointHeight();

            Assert.Equal(650_000, result);
        }
