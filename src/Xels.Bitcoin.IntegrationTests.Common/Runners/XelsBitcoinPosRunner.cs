...
namespace Xels.Bitcoin.IntegrationTests.Common.Runners
{
    public sealed class XelsBitcoinPosRunner : NodeRunner
    {
        ...
        {
            var nodeSettings = new NodeSettings(networksSelector: Networks.Networks.Xlc, protocolVersion: ProtocolVersion.PROVEN_HEADER_VERSION, args: new string[] { $"-datadir={dataDir}", $"-stake={(staking ? 1 : 0)}", "-walletname=dummy", "-walletpassword=dummy" });
