...
namespace NBitcoin.Tests
{
    public class NetworkTests
    {
        ...
        private readonly Network xlcMain;
        private readonly Network xlcTest;
        private readonly Network xlcRegTest;

        public NetworkTests()
        {
            this.networkMain = KnownNetworks.Main;
            this.xlcMain = KnownNetworks.XlcMain;
            this.xlcTest = KnownNetworks.XlcTest;
            this.xlcRegTest = KnownNetworks.XlcRegTest;
        }
        ...
        public void CanGetNetworkFromName()
        {
            ...
            Assert.Equal(NetworkRegistration.GetNetwork("xlcmain"), this.xlcMain);
            Assert.Equal(NetworkRegistration.GetNetwork("XlcMain"), this.xlcMain);
            Assert.Equal(NetworkRegistration.GetNetwork("XlcTest"), this.xlcTest);
            Assert.Equal(NetworkRegistration.GetNetwork("xlctest"), this.xlcTest);
            Assert.Equal(NetworkRegistration.GetNetwork("XlcRegTest"), this.xlcRegTest);
            Assert.Equal(NetworkRegistration.GetNetwork("xlcregtest"), this.xlcRegTest);
        ...
        [Fact]
        public void XlcMainIsInitializedCorrectly()
        {
            ...
            Assert.Equal("XlcMain", network.Name);
            Assert.Equal(XlcNetwork.XlcRootFolderName, network.RootFolderName);
            Assert.Equal(XlcNetwork.XlcDefaultConfigFilename, network.DefaultConfigFilename);
            ...
            Assert.Equal(XlcNetwork.XelsMaxTimeOffsetSeconds, network.MaxTimeOffsetSeconds);
            Assert.Equal(XlcNetwork.XelsDefaultMaxTipAgeInSeconds, network.MaxTipAge);
            Assert.Equal(10000, network.MinTxFee);
            Assert.Equal(10000, network.FallbackFee);
            Assert.Equal(10000, network.MinRelayTxFee);
            Assert.Equal("XLC", network.CoinTicker);

            Assert.Equal(2, network.Bech32Encoders.Length);
            Assert.Equal(new Bech32Encoder("xlc").ToString(), network.Bech32Encoders[(int)Bech32Type.WITNESS_PUBKEY_ADDRESS].ToString());
            Assert.Equal(new Bech32Encoder("xlc").ToString(), network.Bech32Encoders[(int)Bech32Type.WITNESS_SCRIPT_ADDRESS].ToString());
        ...
        [Fact]
        [Trait("UnitTest", "UnitTest")]
        public void XlcTestnetIsInitializedCorrectly()
        {
            ...
            Assert.Equal(10000, network.FallbackFee);
            Assert.Equal(10000, network.MinRelayTxFee);
            Assert.Equal("TXLC", network.CoinTicker);

            Assert.Equal(2, network.Bech32Encoders.Length);
            Assert.Equal(new Bech32Encoder("txlc").ToString(), network.Bech32Encoders[(int)Bech32Type.WITNESS_PUBKEY_ADDRESS].ToString());
            Assert.Equal(new Bech32Encoder("txlc").ToString(), network.Bech32Encoders[(int)Bech32Type.WITNESS_SCRIPT_ADDRESS].ToString());
        ...
        [Fact]
        [Trait("UnitTest", "UnitTest")]
        public void XlcRegTestIsInitializedCorrectly()
        {
            Network network = this.xlcRegTest;

            Assert.Empty(network.Checkpoints);
            Assert.Empty(network.DNSSeeds);
            Assert.Empty(network.SeedNodes);

            Assert.Equal("XlcRegTest", network.Name);
            Assert.Equal(XlcNetwork.XlcRootFolderName, network.RootFolderName);
            Assert.Equal(XlcNetwork.XlcDefaultConfigFilename, network.DefaultConfigFilename);
