...
namespace Xels.Bitcoin.Features.Interop
{
    public class InteropPoller : IDisposable
    {
        ...
        /// <summary>
        /// Retrieves any Transfer events from the logs of the Wrapped Xlc contract deployed on specified chain.
        ...
        private async Task CheckForContractEventsAsync(DestinationChain targetChain, IETHClient chainClient)
        {
            ...
            // Check for all Transfer events against the WrappedXlc contract since the last time we checked.
            // In future this could also poll for other events as the need arises.
            List<EventLog<TransferEventDTO>> transferEvents = await chainClient.GetTransferEventsForWrappedXlcAsync().ConfigureAwait(false);
                ...
                */
                ...
                switch (request.RequestStatus)
                {
                    ...
                    case (ConversionRequestStatus.OriginatorNotSubmitted):
                    {
                        ...
                        BigInteger transactionId = await clientForDestChain.SubmitTransactionAsync(this.interopSettings.ETHSettings.WrappedXlcContractAddress, 0, abiData).ConfigureAwait(false);
