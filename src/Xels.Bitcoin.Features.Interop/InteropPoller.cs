...
namespace Xels.Bitcoin.Features.Interop
{
    public sealed class InteropPoller : IDisposable
    {
        ...
        /// <summary>
        /// Processes burn() contract calls made against the Wrapped Xlc contract deployed on a specific chain.
        /// </summary>
        private void ProcessBurn(string blockHash, string transactionHash, BurnFunction burn)
        {
            ...
            string destinationAddress = burn.XlcAddress;
            ...
            lock (this.repositoryLock)
            {
                this.conversionRequestRepository.Save(new ConversionRequest()
                {
                    ...
                    DestinationChain = DestinationChain.XLC
                });
            }
        }
        ...
        /// the multisig wallet contract on the Ethereum chain. This data consists of a method call to the transfer() method on the wrapped XLC contract,
        ...
        private async Task ProcessConversionRequestsAsync()
        {
            ...
            foreach (ConversionRequest request in mintRequests)
            {
                ...
                // TODO: Perhaps the transactionId coordination should actually be done within the multisig contract. This will however increase gas costs for each mint. Maybe a Cc contract instead?
                    ...
                    case ConversionRequestStatus.OriginatorNotSubmitted:
                        {
                            ...
                            // First construct the necessary transfer() transaction data, utilising the ABI of the wrapped XLC ERC20 contract.
                            ...
                            MultisigTransactionIdentifiers identifiers = await clientForDestChain.SubmitTransactionAsync(this.interopSettings.GetSettingsByChain(request.DestinationChain).WrappedXlcContractAddress, 0, abiData, gasPrice).ConfigureAwait(false);
                ...
                // Instead they are initiated by the user, via a contract call to the burn() method on the WrappedXlc contract.
                // They need to provide a destination XLC address when calling the burn method.

                // Properly processing burn transactions requires emulating a withdrawal on the main chain from the multisig wallet.
                // It will be easier when conversion can be done directly to and from a Cc contract instead.
            }
        }
        ...
        /// <para>
        /// Multisig members on CcTest can use the -overrideoriginator command line parameter to determine who
        ...
        private async Task PerformReplenishmentAsync(ConversionRequest request, BigInteger amountInWei, BigInteger startBalance, bool originator)
        {
            ...
            if (originator)
            {
                ...
                MultisigTransactionIdentifiers identifiers = await this.ethClientProvider.GetClientForChain(request.DestinationChain).SubmitTransactionAsync(this.interopSettings.GetSettingsByChain(request.DestinationChain).WrappedXlcContractAddress, 0, mintData, gasPrice).ConfigureAwait(false);
