...
namespace Xels.Bitcoin.Features.Interop.ETHClient
{
    public interface IETHClient
    {
        ...
        /// In this case the filter is specifically listening for Transfer events emitted by the wrapped xlc
        /// contract.
        /// </summary>
        ...
        Task<List<EventLog<TransferEventDTO>>> GetTransferEventsForWrappedXlcAsync();

        /// <summary>
        /// Retrieves the XLC address that was recorded in the wrapped XLC contract when a given account
        ...
        /// </summary>
        /// <param name="address">The Ethereum account to retrieve the destination XLC address for.</param>
        /// <returns>The XLC address associated with the provided Ethereum account.</returns>
        ...
        /// </summary>
        /// <param name="destination">The account that the transaction is being sent to after confirmation. For wXLC operations this will typically be the wXLC ERC20 contract address.</param>
        /// <param name="value">The amount that is being sent. For wXLC operations this is typically zero, as the balance changes are encoded within the additional data.</param>
        ...
        /// <summary>
        /// Returns the encoded form of transaction data that calls the transfer(address, uint256) method on the WrappedXlc contract.
        ...
        /// </summary>
        /// <param name="address">The address to transfer a quantity of the wXLC token to.</param>
        ...
        /// </summary>
        /// <param name="address">The account that needs tokens to be minted into it (not the address of the multisig contract or the wrapped XLC contract)</param>
        ...
        /// </summary>
        /// <param name="amount">The number of tokens to be minted. This is denominated in wei.</param>
        /// <param name="xlcAddress">The destination address on the XLC chain that the equivalent value of the burnt funds will be sent to.</param>
        /// <returns>The hex data of the encoded parameters.</returns>
        string EncodeBurnParams(BigInteger amount, string xlcAddress);
    }
    ...
    public class ETHClient : IETHClient
    {
        ...
        {
            this.transferEventHandler = this.web3.Eth.GetEvent<TransferEventDTO>(this.settings.WrappedXlcContractAddress);
        ...
        /// <inheritdoc />
        public async Task<List<EventLog<TransferEventDTO>>> GetTransferEventsForWrappedXlcAsync()
        {
            try
            {
        ...
        {
            return await WrappedXlc.GetDestinationAddressAsync(this.web3, this.settings.WrappedXlcContractAddress, address).ConfigureAwait(false);
        }
        ...
        {
            return await WrappedXlc.GetErc20BalanceAsync(this.web3, this.settings.WrappedXlcContractAddress, addressToQuery).ConfigureAwait(false);
        }
        ...
        /// <inheritdoc />
        public string EncodeBurnParams(BigInteger amount, string xlcAddress)
        {
            // TODO: Extract this directly from the ABI
            const string BurnMethod = "7641e6f3";

            var abiEncode = new ABIEncode();
            string result = BurnMethod + abiEncode.GetABIEncoded(new ABIValue("uint256", amount), new ABIValue("string", xlcAddress)).ToHex();
